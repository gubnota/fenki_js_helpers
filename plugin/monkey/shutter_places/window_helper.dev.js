...
(function(window) {
    ...
    var ShutterPlacesWindowHelperScript = (function() {
        ...
        var keys = ['buttonText','closeText'];
        var dic = 
        {'en':['Places','Collapse'],
        'ru':['Места','Свернуть'],
        'zh':['地方','关闭'],
        'fr':['Lieux','Fermer'],
        'pl':['Miejsca','Zwiń okno'],
        'es':['Lugares','Cerrar'],
        'tr':['Yerler','Yakın'],
        'pt':['Locais','Fechar'],
        'de':['Places','Fenster'],
    ...
    var ShutterPlacesWindowHelperScript = (function() {
      ...
      if (ShutterPlacesElement != null) {
        ...
        function Button(_param) {
          ...
          //   if (_buttonText !== undefined)
          //       buttonText = _buttonText;
            if (_param.url !== undefined) {
                url = _param.url;
            }
                var lang = _get_user_lang();
                for (var i in keys) {
                  if (dic[lang][i] !== undefined && _param[keys[i]] === undefined){
                    param[keys[i]] = dic[lang][i];
                  }
                };
                // url = 'http://www.bing.com/translator/?from='+lang+'&to='+to_lang;
//            }
            var el = document.createElement('div');
