...
if(typeof(IFrameWindowHelper) == 'undefined'){
    n.onload = function() {ShutterPlacesWindowHelperScriptPrev.Button({buttonText:"Places",closeText:"Collapse",url:'//gubnota.github.io/fenki_js_helpers/plugin/monkey/shutter_places/response.json'});}
}
...
<script>
  document.addEventListener('DOMContentLoaded', function(event) {
    ShutterPlacesWindowHelperScriptPrev.Button({buttonText:"Places",closeText:"Collapse",url:'//gubnota.github.io/fenki_js_helpers/plugin/monkey/shutter_places/response.json'});
  });
</script>
*/
...
(function(window) {
    ...
    if ( !! window.ShutterPlacesWindowHelperScriptPrev) {
        return window.ShutterPlacesWindowHelperScriptPrev;
    }

    var document = window.document;
    // IE8 does not support textContent, so we should fallback to innerText.
    var ShutterPlacesWindowHelperScriptPrev = (function() {
        var id = Math.ceil(Math.random() * 1000); //уникальный id объекта
        var ShutterPlacesWindowHelperScriptPrevId = 'ShutterPlacesInfo' + id;
        ...
        function _createDialogElement(dismissText, linkHref) {
            ...
            var ShutterPlacesWindowHelperScriptPrevElement = document.createElement('div');
            ShutterPlacesWindowHelperScriptPrevElement.id = ShutterPlacesWindowHelperScriptPrevId;
            ...
            dialog.id = ShutterPlacesWindowHelperScriptPrevId+'dialog';
            ...
            ShutterPlacesWindowHelperScriptPrevElement.appendChild(glassPanel);
            ShutterPlacesWindowHelperScriptPrevElement.appendChild(dialog);
            ShutterPlacesWindowHelperScriptPrevElement.appendChild(css);
            return ShutterPlacesWindowHelperScriptPrevElement;
        }
    ...
    for (var i = 0; i< a.length; i++){
        ...
        function _showShutterPlacesWindowHelperScriptPrev(dismissText, linkHref) {
      var ShutterPlacesElement = document.getElementById(ShutterPlacesWindowHelperScriptPrevId);
      if (ShutterPlacesElement != null) {
        ...
        function Dialog(_buttonText, dismissText, _url) {
            ...
            _showShutterPlacesWindowHelperScriptPrev(dismissText || param.closeText, _url || url);
            ShutterPlacesWindowHelperScriptPrev.draw_table();
            ShutterPlacesWindowHelperScriptPrev.load_info(param.url);
        }
...
//            }
            ...
            css.innerHTML += '#' + buttonId + ':hover{' + ['right:-21px','background:#249cc9','box-shadow:0 2px 4px 0 rgba(71,0,0,.31)'].join(';') + '}';
            el.id = buttonId;
            el.innerHTML = 'Table';//param.buttonText;
...
//                                place = 'https://www.google.com/maps/@' + data[i].latitude + ',' + data[i].longitude + ',11z';
                            ...
                            ShutterPlacesWindowHelperScriptPrev.resolve_place(jQuery('#' + ShutterPlacesIframeId + ' table #ph' + data[i].media_id + '_' + i + ' .place a'), data[i].coordinates[1], data[i].coordinates[0], 11);
                        };
                    },
                    cache: false,
                    crossDomain: true,
        ...
        exports.id = {
            'id': id,
            'ShutterPlacesWindowHelperScriptPrevId': ShutterPlacesWindowHelperScriptPrevId,
    ...
    window.ShutterPlacesWindowHelperScriptPrev = ShutterPlacesWindowHelperScriptPrev;
    return ShutterPlacesWindowHelperScriptPrev;
})(this);